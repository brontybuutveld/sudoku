...
public class AlgorithmX {
    private List<Integer> solution, solution2;
    ...
    public void search(int k, boolean game, boolean print, boolean solcpy, boolean first, boolean early) {
        ...
        if (!input.isEmpty()) {
            handleInput(game);
            if (game) {
                Game g = new Game(solution, head, matrix);
                g.game();
            }
    ...
    /*
     * each row is either covered, or has 4 nodes.
     * therefore it is impossible to double up.
     * and they are spaced in a predictable manner.
     * so no need for spacer nodes either.
     */
    ...
    private void handleInput(boolean game) {
        ...
        try {
            ...
            System.out.println("Make sudokus.txt file in src/main/java/org/example/");
        }
    }
    ...
    private void printFile() throws FileNotFoundException {
        FileOutputStream fos = new FileOutputStream("src/main/java/org/example/sudokus.txt", true);
